...
with st.expander("🔍 Preguntas guía para encontrar tu insight"):
    st.write("**Juega con los filtros del sidebar mientras te haces estas preguntas:**")
    
    st.write("1. **Region menos rentable.** Deja solo una region a la vez en el filtro. ¿Cual tiene el margen mas bajo? ¿Por que crees que sea?")
    st.write("2. **Categoria problema.** ¿Hay alguna categoria cuyo margen sea sospechosamente bajo incluso cuando todas las regiones estan activas? ¿Que decision tomarias como gerente?")
    st.write("3. **Patron temporal.** En el grafico de 'Ventas mensuales', ¿hay meses donde claramente vendemos mas o menos? ¿Que implicaria eso para inventario, contratacion o promociones?")
    st.write("4. **Reto extra.** Si fueras CEO y tuvieras 10 segundos para tomar UNA decision basada en este dashboard, ¿que decision tomarias y por que?")
